...
#### Missing values ####
##if you calculate something with an NA value, your computation will not work
##use is.na(vector) to check for missing values
##use mean(x, na.rm == TRUE) to exclude NA during a calculation
...
#### Data frames ####
##str() = structure of the dataframes
...
dim(mtcars) #shows you the number of dimensions
...
##the below will not turn strings into factors
...
# Hint: use table() ##for each of the character strings, how many observations are there?
table(gapminder$year)

# Create a proportion table of the variable 'continent'
# Hint: use prop.table() ##what proportion of the total are each of a certain variable
...
hist(gapminder07$lifeExp, main = "Life Frequency across all countries in 2007", xlab = "Life Expectancy", ylab = "Frequency")
...
plot(gapminder07$lifeExp ~ gapminder07$gdpPercap, main = "Life Expectancy vs. GDP", xlab = "gdp Per capita", ylab = "Life Expenctacy")
...
abline(h = mean(gapminder07$lifeExp)) ##need to use h for horizontal lines
